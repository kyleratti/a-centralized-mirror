...
public class Script_2024_11_21T13_04_RemoveBadPostId : IDbMaintenanceScript
{
	...
	public async Task Run(IDatabaseConnection<ReadWrite> dbConnection)
	{
		...
		await dbConnection.Execute(
			"""
			BEGIN TRANSACTION;
			DELETE FROM reddit_comments WHERE reddit_post_id = '1gg50tl';
			DELETE FROM link_queue WHERE reddit_post_id = '1gg50tl';
			DELETE FROM links WHERE reddit_post_id = '1gg50tl';
			COMMIT;
			""");
	}
}
